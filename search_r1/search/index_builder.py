...
class Index_Builder:
    ...
    def encode_all(self):
        ...
        for start_idx in tqdm(range(0, len(self.corpus), self.batch_size), desc='Inference Embeddings:'):
            ...
            if "T5" in type(self.encoder).__name__:
                ...
                if "jina" in self.retrieval_method.lower():
                    # Jina-v4 需要指定任务类型
                    output = self.encoder(**inputs, task_label="retrieval.passage", return_dict=True)
                else:
                    output = self.encoder(**inputs, return_dict=True)

                # 兼容不同模型的输出结构；Qwen3 无 pooler_output，改用 last_token 池化
                if "qwen3" in self.retrieval_method.lower():
                    ...
                    embeddings = pooling(pooler_output,
                                         output.last_hidden_state,
                                         inputs['attention_mask'],
                                         self.pooling_method)
    ...
    def build_dense_index(self):
        ...
        if self.faiss_gpu:
            # 先在 CPU 上训练和添加，避免 GPU 显存溢出
            print("Training index on CPU...")
            if not faiss_index.is_trained:
                faiss_index.train(all_embeddings)
            
            print("Adding embeddings to index on CPU (in batches)...")
            # 分批添加，避免一次性占用过多内存
            add_batch_size = 100000
            for i in tqdm(range(0, len(all_embeddings), add_batch_size), desc="Adding to index"):
                end_idx = min(i + add_batch_size, len(all_embeddings))
                faiss_index.add(all_embeddings[i:end_idx])
            
            print("Index built on CPU (GPU will be used during retrieval)")
        else:
            if not faiss_index.is_trained:
                faiss_index.train(all_embeddings)
...
def main():
    ...
    if args.pooling_method is None:
        ...
        if args.pooling_method not in ['mean','cls','pooler', 'last_token']:
